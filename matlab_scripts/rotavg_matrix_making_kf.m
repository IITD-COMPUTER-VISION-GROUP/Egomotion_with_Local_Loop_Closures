...
count=0;
 for i=1:length(Pose_abs_copy)
     [R_2wrt0, T_2wrt0, SE3pose_2wrt0,lie]=concatenatePose(extramatch3(i,3:8),so3(j,2:7));
     so3(j,1)
     Pose_abs_copy(i,3:5)=lie(1:3);
     count=count+1;
 ...
 g=abs(Pose_abs_copy_2-Pose_abs_copy);
 
 for i=23:length(Pose_abs_copy)
     [R_1wrt2, T_1wrt2, SE3pose_1wrt2,lie]=concatenateOriginPose(Pose_abs_copy(i,3:8), Pose_abs_copy(23,3:8));
     Pose_abs_copy(i,3:8)=lie;
 end
 
 dlmwrite('rotavg_kf_24.txt',Pose_abs_copy(23:end,:),' ');
