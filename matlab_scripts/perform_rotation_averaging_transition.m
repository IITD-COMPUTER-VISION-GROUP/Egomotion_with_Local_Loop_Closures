function [ Filtered_pose ] = perform_rotation_averaging_transition(extramatch1,extramatch2,Pose_abs,transition_frame_id)
...
Pose_abs=Pose_abs(mod(Pose_abs(:,2)-transition_frame_id,8)==0,:); %updated condition

%Pose_abs=[transition_frame_id transition_frame_id 0 0 0 0 0 0;Pose_abs];
...
%now starts from 1 
Pose_abs_copy=[transition_frame_id transition_frame_id 0 0 0 0 0 0 0 0;Pose_abs_copy];
Pose_abs_copy_2=[transition_frame_id transition_frame_id 0 0 0 0 0 0 0 0;Pose_abs_copy_2];
extramatch3=[delim;extramatch3];


%!!!!
 for i=2:length(Pose_abs_copy)

     if mod((Pose_abs_copy(i,1)-transition_frame_id),keyframe_prop)==0 %updated condition
        j=j+1;
        Pose_abs_copy(i-1,3:8)=so3(j,2:7); %kf 
     else
        [R_2wrt0, T_2wrt0, SE3pose_2wrt0,lie]=concatenatePose(extramatch3(i,3:8),so3(j,2:7)); %non kf
         Pose_abs_copy(i-1,3:8)=lie(1:6); %updated rotations+trans
     end
     count=count+1;
